...
- **Catálogo / diseños**: `designs` (banco de diseños de Maria Jose, con estado y flag `published_to_ecommerce`), `design_images` (galería de imágenes por diseño, con `is_cover`; `0008_disenos_storage.sql`), `design_requests` (solicitudes de personalizado desde la web, previas a WhatsApp), `products` + `product_variants` (catálogo de stock; `products` tiene `collection_id`/`is_featured`/`is_bestseller` desde `0009_catalogo_colecciones.sql`), `collections` (colecciones temáticas curadas por el admin).
