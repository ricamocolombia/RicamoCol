...
- **Base de datos / Auth / Storage**: Supabase (Postgres) — proyecto real conectado vía `.env.local` en ambas apps (no versionado). Esquema en [`supabase/migrations/0001_init.sql`](./supabase/migrations/0001_init.sql), **ya aplicado** al proyecto real (pegado a mano en el SQL Editor del Dashboard el 2026-08-27; el CLI `supabase link` sigue sin poder usarse porque falta un personal access token o la contraseña de la base de datos). `apps/admin` ya usa Supabase Auth para el login — ver sección 6.
  ...
  - **⚠️ [`supabase/migrations/0007_pagos_produccion.sql`](./supabase/migrations/0007_pagos_produccion.sql) todavía NO está aplicada.** Agrega `accounts_payable.order_id` y las claves de `app_settings` para los proveedores de producción — sin ella, la carga automática de cuentas por pagar desde Ventas y el pago agrupado en Cuentas por pagar fallan.
...
- **Email transaccional**: Resend — `RESEND_API_KEY` configurada (2026-08-29). Correos operacionales del ciclo de vida de un pedido y alertas de inventario ya construidos con marca — ver sección 6 ("Correos transaccionales con marca"). Falta confirmar si el dominio de `RESEND_FROM_EMAIL` está verificado en Resend.
